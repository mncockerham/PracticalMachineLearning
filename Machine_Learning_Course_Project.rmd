...
data <- read.csv("C:/Users/mcockerham/Downloads/pml-training.csv", header = TRUE)
test <- read.csv("C:/Users/mcockerham/Downloads/pml-testing.csv", header = TRUE)
```
...
One way to look at our data is to create a coralation matrix, this shows how the data moves together in our case it is shown as a pie chart and how full the pie is shows how releated to items are.
...
corrgram(cor_matrix, order=TRUE, lower.panel=panel.pie, text.panel=panel.txt,upper.panel=NULL, 
         main="Corralation Matrix") 
         
          
```

Looking at the matrix lets us see that while some fields are highly correlated, we have almos not fields with corralations higher than .9.

Before creating our training and test data sets we need add in the classe column that we are predicting on.
To allow for consistant reruning of the report I am setting a seed for the random number generator before creating the vector that identifies the training and test sets.
...
I then tested it on the testing sample I had exclueded from the training set.
```{r}
...
I then used the supplied R code for the summary assignement to print out the 20 answers.
```{r}
